...
export default function About() {
  ...
  return (
    <div ref={rootRef} className="pb-20 sm:pb-28" aria-label="About Nexus Gaming Cafe">
      {/* Hero */}
      <div className="relative overflow-hidden bg-bg2 border-b border-white/8 pt-20 pb-16 sm:pt-28 sm:pb-24">
        <img
        ...
        <div className="relative container about-hero-anim">
          <SectionHeading
            eyebrow="Our Story"
      ...
      <div className="container">
        {/* Values */}
        <div className="about-values-section mt-12 sm:mt-16 mb-12 sm:mb-16 py-16 sm:py-24">
          ...
          <div className="mt-10 sm:mt-12 grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
            {values.map((v, i) => (
              <div key={v.title} className="about-value-card bg-surface/90 border border-white/8 rounded-xl p-7 sm:p-9 hover:border-white/25 card-glow hover-lift transition-all duration-300">
                <div className="font-display text-[11px] font-semibold uppercase tracking-widest text-lime mb-3.5">
                  0{i + 1}
                </div>
                <h3 className="font-display font-bold text-xl sm:text-2xl text-text mb-3">{v.title}</h3>
                <p className="text-muted text-base leading-relaxed">{v.body}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Timeline */}
        <div className="about-timeline-section mt-12 sm:mt-16 mb-12 sm:mb-16 py-16 sm:py-24 border-t border-white/8">
          <SectionHeading eyebrow="Timeline" title="How We Got Here" />
          <div className="mt-12 sm:mt-16 flex flex-col gap-0 max-w-3xl">
            {milestones.map((m, i) => (
              <div key={m.year} className="about-timeline-item flex gap-6 sm:gap-8">
                <div className="flex flex-col items-center">
                  <div className="w-12 h-12 rounded-lg bg-surface border border-lime/40 flex items-center justify-center flex-shrink-0 shadow-[0_0_12px_rgba(183,255,60,0.1)]">
                    <span className="font-display font-bold text-sm text-lime">{m.year.slice(2)}</span>
                  </div>
                  {i < milestones.length - 1 && <div className="w-px bg-white/10 flex-1 my-3" />}
                </div>
                <div className="pb-10 sm:pb-12">
                  <div className="font-display text-[10px] font-semibold uppercase tracking-widest text-muted mb-1.5">{m.year}</div>
                  <h3 className="font-display font-bold text-xl text-text mb-2.5">{m.title}</h3>
                  <p className="text-muted text-base leading-relaxed">{m.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* The Space & Stats */}
        <div className="mt-12 sm:mt-16 mb-12 sm:mb-16 border-t border-white/8 py-16 sm:py-24">
            ...
            <div>
              ...
              <div className="mt-8 sm:mt-10 grid grid-cols-2 gap-4 sm:gap-5">
                {[['20+', 'Gaming Stations'], ['4', 'Specialized Zones'], ['80+', 'Game Library'], ['₹50K+', 'Monthly Prize Pools']].map(([v, l]) => (
                  <div key={l} className="bg-surface border border-white/8 rounded-xl p-5 sm:p-6 hover:border-lime/20 transition-colors card-glow">
                    <div className="stat-num text-2xl sm:text-3xl">{v}</div>
                    <div className="text-muted text-[11px] font-display font-medium uppercase tracking-wider mt-1">{l}</div>
                  </div>
                ))}
              </div>
            </div>
            <div className="relative h-72 sm:h-80 lg:h-96 rounded-xl overflow-hidden bg-bg2 border border-white/10">
              <img
